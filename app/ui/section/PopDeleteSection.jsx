...
const PopDeleteSection = ({id , onCansle , axios,restartData }) => {
    const handelDelete = async () => {
        try {
          ...
          await axios.delete(`/api/v1/Teacher/section/${id}`);
          onCansle();
          restartData()
        } catch (error) {
...
return (
    ...
    <h1 className='mb-6 text-2xl md:text-3xl text-center text-color-red'>هل انت متأكد من حذف هذه الجلسة </h1>
    <p className="text-color-text2 bg-side-color p-2 mb-4">بعد تأكيد الحذف سيتم حذف   موعد هذه الجلسة لدى جميع الطلاب</p>
    <div >
...
export default PopDeleteSection