...
const PopUpdateAttendes = ({onCansle, update, axios, setData,data ,id}) => {
    const [status, setStatus] = useState ( "" )
    useEffect(() => {
        let attendance = data.find(item => item.id === id);
        if (attendance) {
    ...
    const handelUpdate = async () => {
        ...
        try {
            await axios.put ( `/api/v1/Teacher/attendance`, body );
    ...
    return (
        <div
            className=" fixed  w-[90%] md:w-1/2 p-4 rounded-lg top-[55%] md:top-1/2 left-1/2 center bg-side4-color z-40 direction_rtl">
       ...
       <InputAddClass onChange={setScore} value={score}  lable="ادخل الدرجة الجديدة" type="number"/> */}
            <div className="mt-12">
                <select value={status} onChange={(e) => setStatus ( e.target.value )} id="countries"
            ...
            <div className="flex justify-between px-4 mt-4">
                <button onClick={handelUpdate} type="button"
                        className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-lg px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900">
                    تأكيد
                </button>
                <button onClick={onCansle} type="button"
