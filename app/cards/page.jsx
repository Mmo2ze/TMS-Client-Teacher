...
const handleChange = async (event) => {
  ...
  try {
    ...
    setClassIds((prevClassIds) => [...prevClassIds, ...studentsForClass]);
      ...
      for (let row = 0; row < rowsPerPage; row++) {
        ...
        if (studentIndex < studentsClass.length) {
          const student = studentsClass[studentIndex];
            ...
            pdf.text((student.id && student.id.toString()) || "", nameXPos + 10, nameYPos + 15);
 ...
 {classes.map((cl) => (
  <li onChange={handleChange}>
    <div className="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
      ...
      />
      <label htmlFor={`checkbox-item-${cl.id}`} className="w-full ms-2 text-sm font-medium text-gray-900 rounded dark:text-gray-300">
        {cl.name}
      </label>
    </div>
  </li>
